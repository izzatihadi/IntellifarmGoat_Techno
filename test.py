...
def determine_secondary_health(row):
...
# Determine the secondary health condition
data['Potential Disease'] = data.apply(determine_secondary_health, axis=1)

# Save the updated data with 'Y', 'Health Status' columns to a new Excel file
data.to_excel('Resultsss.xlsx', index=False)